...
function key2(){
    ...
    if [[ $key == "1" ]]; then
        ...
        curl -s https://pastebin.com/raw/mcg0BcFi > /etc/github/api
...
res7() {
wget ${REPO}menu/menu.sh && chmod +x menu.sh && ./menu.sh
clear
}
...
MYIP=$(curl -sS ipv4.icanhazip.com)
IZIN=$(curl -sS https://raw.githubusercontent.com/Sulastriii/izin/main/ip | grep $MYIP | awk '{print $3}' )
d1=$(date -d "$IZIN" +%s)
d2=$(date -d "$today" +%s)
EXP=$(( (d1 - d2) / 86400 ))
...
"'&reply_markup={"inline_keyboard":[[{"text":"🔥ᴏʀᴅᴇʀ","url":"https://t.me/HendraStore58"},{"text":"🔥GRUP","url":"https://t.me/n"}]]}'
